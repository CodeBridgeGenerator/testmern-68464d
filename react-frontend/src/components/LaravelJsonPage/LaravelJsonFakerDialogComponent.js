...
import laravelJsonFakerFactory from "./laravelJsonFakerFactory";
...
const LaravelJsonFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = laravelJsonFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("laravelJson").create(elem));
...
export default connect(mapState, mapDispatch)(LaravelJsonFakerDialogComponent);
