...
import seederData from "./laravelJson.seeder.json";
...
const LaravelJsonSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("laravelJson").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(LaravelJsonSeederDialogComponent);
